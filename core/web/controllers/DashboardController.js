...
exports.mainPost = function(req, res) {
    ...
    if (action == "load") {
        ...
        queryTwo.equalTo("isApproved", 0);
        queryTwo.count().then(function (news) {
            a = news;
            var queryThree = new Parse.Query("EventStructure");
            queryThree.equalTo("isApproved", 0);
            return queryThree.count();
        }).then(function(event) {
            b = event;
            var queryFour = new Parse.Query("CommunityServiceStructure");
            queryFour.equalTo("isApproved", 0);
            return queryFour.count();
        }).then(function(community) {
            c = community;
            var queryFive = new Parse.Query("UserRegisterStructure");
            return queryFive.count();
        }).then(function(user) {
            d = user;
            res.send({news:a,events:b,community:c,user:d});
        });
    }
};
